...
- [x] Run `npm run check && npx vitest run` once more from a clean tree; confirm 0 errors and every test passes. (0 errors/1 warning baseline, 65 files / 1203 tests passed)
- [x] Run `cd src-tauri && cargo test` to confirm the Rust migration tests pass alongside the rest of the Rust test suite. (155 passed, 0 failed)
- [x] Manual Tauri check (`npm run tauri:dev` or a debug build): create a project, add several tasks with different priorities/due dates, drag a card between columns, use the keyboard "Move to…" control, reorder within a column with the up/down buttons, edit and delete a task, and start a focus session from a task — confirm the session starts immediately, is tagged with the task's project, and the due-date overdue styling renders correctly for a past date. (Driven via `npm run dev` in a real browser against the in-memory repository backend, not `tauri:dev` — `repository.ts` picks `memoryRepository` outside Tauri, exercising the same Svelte UI/logic. All steps verified: overdue styling, drag-and-drop, keyboard move, up/down reorder, edit/delete, focus session started immediately and tagged with the task's project in Session History.)
