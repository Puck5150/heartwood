...
describe('createNativeNotificationAdapter', () => {
  ...
  it('focusMainWindow shows, unminimizes, and focuses the existing window', async () => {
  ...
  it('focusMainWindow resolves without throwing when the window port fails', async () => {
    const adapter = createNativeNotificationAdapter({
      isTauriFn: () => true,
      loadWindow: async () => {
