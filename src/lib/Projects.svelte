...
<style>
  ...
  .eyebrow {
    margin: 0;
    /* Was a <p>, then a real <h1> styled as a small-caps kicker (see the
       a11y audit — this was the page's only title-equivalent element, so
       it had to be the heading, not a label above one). A heading that
       reads as a kicker is still a kicker — sized up to an actual display
       headline instead, matching Focus/Greenhouse/History's own
       treatment. */
